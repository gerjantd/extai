...
async function loadFunction({ filePath, functionName, defaultFunctionName = 'func', basePath = cliState_1.default.basePath, useCache = true, }) {
    ...
    if (!(0, file_1.isJavascriptFile)(resolvedPath) && !resolvedPath.endsWith('.py')) {
        throw new Error(`File must be a JavaScript (${file_1.JAVASCRIPT_EXTENSIONS.join(', ')}) or Python (.py) file`);
    }
    try {
        let func;
        if ((0, file_1.isJavascriptFile)(resolvedPath)) {
