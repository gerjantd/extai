...
function renderSystemPrompts(nunjucks, goal) {
    return {
        redteamSystemPrompt: nunjucks.renderString(prompts_1.ATTACKER_SYSTEM_PROMPT, { goal }),
 ...
 */
async function evaluateResponse(redteamProvider, judgeSystemPrompt, targetResponse, previousBestResponse, containsPenalizedPhrase) {
 ...
 */
function updateRedteamHistory(targetResponse, goal, score, isOnTopic, containsPenalizedPhrase, explanation) {
    return {
        role: 'user',
        content: (0, dedent_1.default) `
 ...
 */
async function runRedteamConversation({ context, filters, injectVar, options, prompt, redteamProvider, targetProvider, test, vars, }) {
    ...
    for (let depth = 0; depth < MAX_DEPTH; depth++) {
        ...
        for (const node of currentBestNodes) {
            ...
            for (let i = 0; i < BRANCHING_FACTOR; i++) {
                ...
                const { score, explanation } = await evaluateResponse(redteamProvider, judgeSystemPrompt, targetResponse.output, bestResponse, containsPenalizedPhrase);
                ...
                redteamHistory.push(updateRedteamHistory(targetResponse.output, goal, score, isOnTopic, containsPenalizedPhrase, explanation));
                treeOutputs.push({
                    depth,
                    graderPassed,
                    id: (0, uuid_1.v4)(),
